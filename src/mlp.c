...
void create_model(Model * model, size_t size_batch){    
    size_t size_model_params = SIZE_VOCAB * DIM_EMBEDDINGS // embedding table
    + SIZE_HIDDEN * SIZE_BLOCK * DIM_EMBEDDINGS // hidden weights
    + SIZE_HIDDEN // hidden biases
    + SIZE_HIDDEN * SIZE_VOCAB // output weights
    + SIZE_VOCAB; // output biases
    ...
    size_t size_model_gradients = size_model_params + size_batch * 
...
void softmax_foward(const float * logits, float * probs, size_t size_batch){
    for (size_t idx_sample = 0; idx_sample < size_batch; idx_sample++){
        size_t offset_sample_start = idx_sample * SIZE_VOCAB;
        float max_logit = logits[offset_sample_start];
        for (size_t idx_logit = 0; idx_logit < SIZE_VOCAB; idx_logit++){
            size_t offset_logit = offset_sample_start + idx_logit;
...
float cross_entropy_loss(float * probs, char * labels, size_t size_batch){
    ...
    for (int idx_sample = 0; idx_sample < size_batch; idx_sample++){
        size_t offset_predicted_prob_for_expected_token = idx_sample * SIZE_VOCAB + encode(labels[idx_sample]);
...
void model_backwards(Model * model, TrainingSet * training_set){
    ...
    double time_spent;
    begin = clock();
    size_t size_params = get_size_model_params();
...
void save_model(Model * model, const char *filename){
    ...
    fwrite(model->parameters.table_embedding, sizeof(float), get_size_model_params(), file);
...
void load_model_from_file(Model * model, const char *filename){
    ...
    fread(model->parameters.table_embedding, sizeof(float), get_size_model_params(), file);
    fclose(file);
}
